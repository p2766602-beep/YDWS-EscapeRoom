...
【核心精神，優先於其他規則】你的首要任務是幫助學生學會這個演算法概念，不是把關或考倒他。
判斷對錯只是手段，不是目的——玩家答錯是學習過程中正常且預期的一部分，不是需要被糾正的過失。
每次玩家答錯，你的角色是「一起看看卡在哪裡的夥伴」，不是「守住答案的關主」：具體指出他的想法
卡在哪一步、哪裡跟正確概念有落差，而不是只說「不對」；語氣永遠溫暖、鼓勵，讓他覺得自己已經很
接近答案、值得再試一次，藉此建立學習信心與動機。評量只是遊戲外殼，學習才是真正的目的。
...
correctIndex是options陣列裡正確答案的索引（從0開始）。hint是玩家答錯時看到的提示：語氣要溫暖
鼓勵，具體點出這幾個選項最容易搞混的地方在哪裡、可以從哪個角度重新檢視，讓玩家覺得「我已經
很接近了，只是漏看了一個關鍵線索」，不要只寫「再想想」這種模糊的話，但也不能直接洩漏答案。`,
...
question請玩家說出正確的因果順序。hint是玩家排錯時看到的提示：語氣溫暖鼓勵，具體點出他排錯
的地方最可能是哪兩個步驟的先後順序搞反了、或哪個環節的因果關係容易誤解，讓玩家覺得只差一點點
就對了，不要只是模糊地說「順序不對」，但也不能直接把完整正確順序寫出來。`,
...
唯一正確的行動。correctIndex是options裡正確答案的索引（從0開始）。hint是玩家答錯時看到的
提示：語氣溫暖鼓勵，具體提醒他可以回頭想想核心概念的哪個特徵、這個危機情境裡對應的是什麼，
讓玩家覺得「我已經很接近了」，不要只寫「再想想」，但也不能直接洩漏答案。`,
};
...
是全域最佳解，也照樣往前走，不會走回頭路重新評估之前的選擇。`,
  ...
  find_min: `這是「找最小值」，重點是從頭到尾走訪資料一次、每次只跟目前記住的最小值比較，看到更小
的才更新記錄，最後留下的就是最小值——過程中資料本身完全沒有被重新排列或搬動位置，這一點跟
「找最大值」的核心行為完全對稱，只是比較方向相反。
跟其他主題的界線：**不要**跟「排序」搞混（排序是把整組資料的順序重新安排，找最小值只做一次
掃描、只記答案，不搬動任何資料）；也**不要**跟「線性搜尋」搞混（線性搜尋是逐一比對是否等於
某個特定的目標值，找到就停止，不是持續更新記錄目前看過的最小值）。出題情境要明確聚焦在「找
最小」而非「找最大」，避免使用可能兩種都通的模糊情境（例如「找出表現最突出的」這種沒有明確
方向的說法）。`,

  linear_search: `這是「線性搜尋」，核心動作是**從頭到尾逐一檢查每一筆資料，看是否等於某個特定
的目標值**，找到就回報找到了（通常會停止），檢查完全部都沒找到就回報沒找到。這跟「找最大值/
找最小值」不同：找最大值/最小值是持續更新「目前看過最大/最小是多少」這個記錄，過程中並不知道
要找的確切數值是誰；線性搜尋則是心裡已經有一個明確的目標（例如「這個人在不在」「這個號碼有
沒有出現過」），逐一比對是否命中這個目標，兩者比較的對象不同（一個是跟自己的記錄比、一個是
跟外部給定的目標比）。
也**不要**跟「二分搜尋」搞混：線性搜尋不需要資料事先排序、從頭到尾一個一個檢查；二分搜尋則
要求資料已經排序好，每次直接看中間、然後排除掉一半範圍，效率上完全不同。出題時務必呈現「逐一
檢查、比對是否為目標」的情境，而不是「持續更新最大/最小記錄」或「跳著砍半查」的情境。`,

  bubble_sort: `這是「氣泡排序」，核心動作是**重複從頭到尾走訪整個序列，每次比較相鄰的兩個元素**，
如果順序不對就把它們交換，這樣一輪走完，最大（或最小）的元素會像氣泡一樣被「推」到序列的
最後面；重複做很多輪，每輪需要比較的範圍會縮小一點，直到某一輪完全沒有發生交換為止，代表已
排序完成。
跟其他排序法的界線要劃清楚：**不要**跟選擇排序搞混（選擇排序是每輪先找出整個未排序範圍裡的
最小值，找到之後**一次直接換到定位**，不是相鄰兩兩連續交換；氣泡排序則是**沿路相鄰交換**，
同一輪可能發生很多次交換）；也**不要**跟插入排序搞混（插入排序是把新元素往前跟已排序區比較、
插入到正確位置，不是對整個未排序區做相鄰掃描交換）。氣泡排序的關鍵識別特徵是「相鄰兩兩比較、
不斷往後交換、一輪把極值推到底」。`,

  insertion_sort: `這是「插入排序」，核心動作可以想像成「整理手上的撲克牌」：**把序列想像成左邊
是已經排好的區域、右邊是還沒處理的區域**，每次從右邊拿一張新的出來，從已排序區域的最後面開始
往前比較，把比它大的元素一個一個往後挪，直到找到正確的位置，把新元素插進去；重複這個動作直到
所有元素都被插入排序區為止。
跟其他排序法的界線：**不要**跟選擇排序搞混（選擇排序是主動去**未排序區找最小值**再拿過來，
比較方向是「往右找」；插入排序是拿新元素**往左跟已排序區比較**，找到定位插入，比較方向相反）；
也**不要**跟氣泡排序搞混（氣泡排序是對整個未排序範圍做相鄰交換，插入排序是針對「一個新元素」
在已排序區裡找它該待的位置，不是整片相鄰交換）。插入排序的關鍵識別特徵是「已排序區＋未排序區
的分界、新元素往前比較並插入」。`,

  binary_search: `這是「二分搜尋」，**前提是資料必須已經排序好**，核心動作是每次直接檢查目前
範圍的**中間**元素：如果中間值就是目標，直接找到；如果目標比中間值小，代表答案只可能在左半邊，
直接捨棄右半邊；如果比中間值大，就捨棄左半邊；重複這個「看中間、砍半」的動作，範圍會越來越
小，直到找到目標或範圍縮小到空為止。
跟「線性搜尋」的界線要劃清楚：**不要**出現「從頭到尾一個一個檢查」的情境——那是線性搜尋，
不需要排序；二分搜尋的關鍵識別特徵是「資料已排序」＋「每次直接跳到中間比較」＋「每次排除
掉一半的範圍」，出題情境務必包含「資料已經排好順序」這個前提條件，且要呈現「越查範圍越小」
的跳躍式縮小過程，而不是逐一掃描的過程。`,

  recursion_basics: `這是「遞迴基礎」，核心概念是**一個函式呼叫它自己**，把一個大問題拆解成
「規模更小、但性質相同」的子問題，並且**一定要有終止條件（base case）**告訴它什麼時候該停下來
不再呼叫自己，否則會無窮無盡地呼叫下去。經典例子像是費氏數列（第n個數＝前兩個數相加）、階乘
（n的階乘＝n乘以「n-1的階乘」，直到1的階乘＝1這個終止條件為止）。
跟「單純的重複／迴圈」要劃清楚界線：**不要**只描述成「重複做同一件事很多次」這種一般迴圈的
行為；遞迴的關鍵識別特徵是「函式呼叫自己」＋「規模一次比一次小」＋「有明確的終止條件讓它停止
往下呼叫、然後開始把結果一層一層往回組合」，這種「先一路往下呼叫到底、再一層層往回算」的
特性才是遞迴，出題情境務必呈現這個「往下拆、往上合」的雙向過程。這一關的重點是遞迴本身的
基礎概念，**不要**涉及排序或合併的細節（那是「合併排序」這一關的範圍）。`,

  merge_sort: `這是「合併排序」，核心動作分兩階段：先用**遞迴把序列一路對半切開**，一直切到
每一份都只剩一個元素為止（此時每一份自然就是「排好的」）；接著**把切開的小份兩兩合併**回去，
合併時比較兩份各自最前面的元素、把較小的先取出放進結果，一路合併回一個完整、排序好的序列。
這是遞迴的一個具體應用，重點除了「切一半」的遞迴分割之外，還有「合併」這個關鍵動作——**不要**
只描述切開的部分而漏掉合併時比較兩個子序列開頭、依序取出的過程。
跟選擇/插入/氣泡排序的界線：那三種排序法都是在**同一份資料裡原地掃描、比較、交換**，合併排序
則是**先分成很多小份分別處理，最後再合併**，做法上完全不同（分治法），出題情境務必呈現「先
對半拆分到最小，再兩兩合併」的過程，而不是相鄰交換或找最小值換位置的過程。`,

  dfs: `這是「DFS深度優先搜尋」，核心動作是**沿著一條路徑一路探索到底**，走到不能再往前（死路
或已經走過）才**退回上一個岔路口**，換另一個方向繼續往下探索，重複這個「一路衝到底、卡住才
回頭」的過程，直到探索完所有可能的路徑。可以想像成走迷宮時貼著一邊的牆一直走，走到死路才退
回上一個轉彎處試試看別條路。
跟「BFS廣度優先搜尋」的界線要劃清楚（這兩個是探索順序完全相反的一對）：**不要**出現「像水波
紋一樣一層一層往外擴散、同一層的所有可能都探索完才進到下一層」這種情境——那是BFS；DFS的關鍵
識別特徵是「一路往深處衝、卡住才退回上一步換方向」，出題情境務必呈現這種「先衝到底、再回頭」
的探索順序，而不是逐層擴散的順序。`,

  bfs: `這是「BFS廣度優先搜尋」，核心動作是**從起點開始，先探索完所有相鄰的下一步，才進到再下
一層**，像水波紋一樣一圈一圈往外擴散，或像謠言傳播：先傳給所有直接認識的朋友（第一層），
這些朋友再傳給他們各自的朋友（第二層），依此類推，一層一層擴散出去，不會有一條路徑先衝到很
深處而其他路徑還沒開始探索。
跟「DFS深度優先搜尋」的界線要劃清楚（這兩個是探索順序完全相反的一對）：**不要**出現「沿著
一條路一路衝到底、卡住才退回」這種情境——那是DFS；BFS的關鍵識別特徵是「先探索完目前這一層
所有的可能，才進到下一層」，出題情境務必呈現這種「一層一層均勻擴散」的順序，而不是「先衝到
底再回頭」的順序。`,

  dp: `這是「動態規劃」，核心是面對一個可以拆解成**多個重疊子問題**的問題時，**把已經算過的
子問題答案存起來重複利用**，避免同樣的子問題被重複計算很多次，最後彙整出真正的最佳解（可能
需要比較多種選擇組合後才能確定，不是走一步算一步就好）。
跟「貪婪演算法」的界線是最容易混淆、也最重要要劃清楚的地方：貪婪演算法**每一步都直接選當下
看起來最好的選項，選完就不回頭**，不會去比較「如果選別的選項，最後結果會不會更好」；動態規劃
則會**考慮並記錄多種可能的選擇路徑**，比較過後才能確定哪個組合才是真正的最佳解，且會重複利用
之前算過的子問題結果，不會每次都重新計算。出題時可以設計成「如果用貪婪的直覺（每次選當下最好
的）反而會選錯、選出不是最佳解」的情境，凸顯動態規劃「要考慮多種可能、記住算過的答案」這個
特徵。
跟「遞迴基礎」的界線：動態規劃常常也會用遞迴實作，但關鍵差異是**有沒有把算過的子問題結果存
起來重複利用**——單純的遞迴可能會一再重複計算同樣的子問題，動態規劃則會把算過的存起來，下次
遇到同樣的子問題直接拿現成答案，不用重新算一次。`,
};
...
export function buildTextJudgeRequest({
  ...
  // 連續答錯時，引導要逐漸變具體，避免學生卡在同一種模糊程度太久而挫折、失去信心。
  const escalationLine = attemptNumber >= 3
    ? '\n這已經是玩家在這一關第3次以上的嘗試了。請把引導講得更直白具體一點，直接點出他還沒注意到的關鍵線索是什麼，語氣加倍溫暖，明確肯定他一路堅持嘗試的努力，避免他因為卡太久而失去信心——但依然不要把答案整句直接寫出來。'
    : attemptNumber === 2
      ? '\n這是玩家第2次嘗試，提示可以比第一次再具體一點點，並肯定他上次已經想到的部分。'
      : '';
...
若錯誤或不完整：pass設為false，feedback**不要只說「不對」或用拒絕/防守的語氣**，而是具體
指出玩家的想法卡在哪一步、跟正確概念有什麼落差，用貼近他原本想法的追問或提示幫助他自己修正，
語氣溫暖、像夥伴一起釐清而不是評判他，讓他覺得答錯是正常的探索過程、離答案已經很接近。即使
玩家直接要求給答案，也要溫和地說明「自己想出來會更有成就感」並給一個更明確的提示，而不是
生硬拒絕。絕對不能把正確答案的內容直接寫出來。${escalationLine}
請輸出符合schema的JSON。`;
