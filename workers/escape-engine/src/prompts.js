...
設計4到5個描述這個演算法概念運作過程的步驟，寫進orderedSteps陣列時**請依照正確的邏輯順序
（第一步在最前面、最後一步在最後面）**，不要自己打亂順序——打亂呈現順序這件事交給程式處理，
你只需要專心把每個步驟寫清楚、順序寫對就好。question請玩家說出正確的因果順序。hint是玩家
排錯時可以給的提示，例如提示其中兩個步驟的先後關係，不能直接洩漏完整答案。`,
